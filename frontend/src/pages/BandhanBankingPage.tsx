...
import { BandhanBankingUI } from '../components2_bank/banking/BandhanBankingUI';
...
export default function BandhanBankingPage() {
    ...
    const handleConnect = useCallback(async () => {
        ...
        try {
            ...
            const url = `${TOKEN_ENDPOINT}?name=${userId}&agent=bandhan_banking`;
                    ...
                    <div className="fixed bottom-24 md:bottom-8 right-4 md:right-8 z-50 flex flex-col items-end gap-4">
                        ...
                        {!isCardOpen && (
                            <button
                                ...
                                disabled={connecting}
                                className="pointer-events-auto group relative w-16 h-16 rounded-full bg-gradient-to-br from-[#fbaf17] to-[#e59e15] shadow-[0_4px_20px_rgba(251,175,23,0.4)] flex items-center justify-center transition-transform hover:scale-110 active:scale-95 disabled:scale-100 disabled:opacity-80 border-4 border-white/20"
                            >
                                {/* Tooltip */}
                                <div className="absolute right-full mr-4 bg-black/80 backdrop-blur text-[#fbaf17] px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none border border-[#fbaf17]/20">
                                    Ask Bandhan Assistant
                                </div>

                                <div className="relative z-10">
                                    <Bot size={32} className="text-[#002d5e] animate-pulse" />
                                </div>

                                {/* Ping animation */}
                                <div className="absolute inset-0 rounded-full bg-[#fbaf17] opacity-50 animate-ping"></div>
                            </button>
                        )}
                    </div>
                )}
